  ...
  static async tourScheduleAction(req, res, next) {
    ...
    try {
      if (action === "invite") {
        ...
        result = {
          data: {
            message: "successfully invite some users to tour!",
          },
        };
      } else if (action === "join") {
        result = await UserTourSchedule.create({
          UserId,
          TourScheduleId: id,
          role: "member",
          status: "pending",
        ...
        result = await UserTourSchedule.update(
          {
            status: action,
          },
          { where: { TourScheduleId: id, UserId }, returning: true }
        );
      }
      res.status(200).json(result);
    } catch (error) {
